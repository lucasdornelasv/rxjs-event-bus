import { Observable, Subject, Subscription, asyncScheduler, defer, merge } from 'rxjs';
...
export class RxEventBus {
  ...
  emit(eventType: string, payload?: any) {
    this._subjectsMap.get(eventType)?.next({ eventType, payload });
  }
  ...
  private _getOrCreateObservable(eventType: string): Observable<any> {
    ...
    if (this._observablesMap.has(eventType)) {
		...
		observable = this._getOrCreateSubject(eventType).pipe(finalize(() => {
